...
const customersStories = [
	{
		...
		author: {
			...
			avatar: "/avatars/customer-01.png",
		},
	},
	{
		content:
		...
		author: {
			name: "Jacob Jones",
			role: "CEO na JJ Org",
			avatar: "/avatars/customer-02.png",
		},
	},
];
		...
		<section className="container py-8 md:py-10">
			<div className="flex flex-col items-center gap-12">
				...
				<div className="grid gap-6 md:grid-cols-2">
					{customersStories.map((story, index) => (
						<div key={story.author.name} className="flex flex-col gap-6 rounded-lg bg-gray-600 p-6 md:p-12 border border-gray-400">
							<p key={index} className="text-body-md text-gray-200 italic">
								{story.content}
							</p>
							<div className="flex items-center gap-3">
								<div className="relative h-10 w-10 overflow-hidden rounded-full">
									<Image src={story.author.avatar} alt={story.author.name} fill className="rounded-full" />
								</div>
								<div className="flex flex-col">
									<strong className="text-gray-200 text-sm">{story.author.name}</strong>
